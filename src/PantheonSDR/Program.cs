...
//   PantheonSDR              → service mode (LAN HTTP + HTTPS, console banner).
//                                 Headless-friendly — what a Raspberry-Pi-shack or
//                                 a Docker container runs.
//   PantheonSDR --desktop    → Photino shell (loopback HTTP for the webview,
//                                 plus LAN HTTPS so a phone can pick up the
//                                 session while the operator is away from the
//                                 shack PC — see ShareOverLan).
//   PantheonSDR --server     → service mode + a small Photino status window
...
{
    // PantheonSDR.csproj defaults to OutputType=Exe (console subsystem) so that
    ...
    private static int RunDesktop(string[] args)
    {
        ...
        Console.WriteLine($"PantheonSDR (desktop) hosting backend at {startUrl}");
        ...
        {
            Console.WriteLine($"PantheonSDR (desktop) LAN share: https://{ip}:{lanHttpsPort}");
        }
